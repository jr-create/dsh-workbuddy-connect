...
*/
const WINDOWS_ELECTRON_SUFFIX = ["WorkBuddy", "WorkBuddy.exe"];
/**
...
*/
function windowsElectronCandidates() {
	const roots = [];
	const localAppData = process.env["LOCALAPPDATA"]?.trim();
	if (localAppData !== void 0 && localAppData !== "") roots.push(join(localAppData, "Programs"));
	for (const name of ["ProgramFiles", "ProgramFiles(x86)"]) {
		const root = process.env[name]?.trim();
		if (root !== void 0 && root !== "") roots.push(root);
	}
	const candidates = roots.map((root) => join(root, ...WINDOWS_ELECTRON_SUFFIX));
	for (let code = "C".charCodeAt(0); code <= "Z".charCodeAt(0); code += 1) candidates.push(join(`${String.fromCharCode(code)}:\\`, "Program Files", ...WINDOWS_ELECTRON_SUFFIX));
...
function defaultWorkBuddyElectronPath() {
	...
	if (process.platform === "win32") return windowsElectronCandidates().find((candidate) => isExecutable(candidate));
}
