...
/**
 * The Windows install's own layout, relative to whichever root it chose.
 *
 * WorkBuddy ships as a self-contained Electron app: one folder holding the
 * main executable, which is the binary that can run with
 * `ELECTRON_RUN_AS_NODE=1`.
 */
const WINDOWS_ELECTRON_SUFFIX = ['WorkBuddy', 'WorkBuddy.exe'] as const

/**
 * Windows roots worth probing, in probe order.
 *
 * There is no single install root to name: the updater lets the user pick a
 * drive, so a non-system one is a common choice (this machine has the app on
 * `D:`), and the uninstall entry reports no `InstallLocation` to read instead.
 * The sweep is therefore env-root-first, then one `X:\Program Files` per drive
 * letter. Each candidate costs a single stat, and a binary that exists but is
 * not the right install is still caught downstream: the key it returns is
 * matched against the envelope's key id before anything is decrypted.
 */
function windowsElectronCandidates(): string[] {
  const roots: string[] = []
  const localAppData = process.env['LOCALAPPDATA']?.trim()
  // The updater's own default for a per-user install.
  if (localAppData !== undefined && localAppData !== '') roots.push(join(localAppData, 'Programs'))
  for (const name of ['ProgramFiles', 'ProgramFiles(x86)'] as const) {
    const root = process.env[name]?.trim()
    if (root !== undefined && root !== '') roots.push(root)
  }
  const candidates = roots.map(root => join(root, ...WINDOWS_ELECTRON_SUFFIX))
  // Drive letters are joined onto a rooted `X:\` rather than a bare `X:`:
  // `join('D:', 'Program Files')` yields the drive-relative `D:Program Files`,
  // which resolves against the current directory instead of the drive's root.
  for (let code = 'C'.charCodeAt(0); code <= 'Z'.charCodeAt(0); code += 1) {
    candidates.push(join(`${String.fromCharCode(code)}:\\`, 'Program Files', ...WINDOWS_ELECTRON_SUFFIX))
  }
 ...
 *
 * macOS has one verified layout. Windows probes the install roots above and
 * returns the first that is executable, so an ordinary install needs no
 * configuration; Linux still has to set {@link WORKBUDDY_ELECTRON_BIN_ENV}
 * explicitly, because no layout has been confirmed there and guessing would
 * spawn the wrong app's binary.
 */
export function defaultWorkBuddyElectronPath(): string | undefined {
  if (process.platform === 'darwin') return MACOS_ELECTRON_PATH
  if (process.platform === 'win32') {
    return windowsElectronCandidates().find(candidate => isExecutable(candidate))
  }
  return undefined
}
